...
- **Host Command Sandbox (on by default, no containers)**: commands the agent runs (the bash tool, and ACP terminals and skill tools by default) are confined per spawn with OS-native mechanisms — Landlock + seccomp (plus bubblewrap when available) on Linux, Seatbelt (`sandbox-exec`) on macOS — to the workspace, temp dirs and dependency caches. Pando's own config, `.pando/` and `.git/hooks` stay read-only, credential-looking env vars are scrubbed, and a blocked command gets a `[sandbox]` hint plus an explicit-approval path to re-run once outside the sandbox. Switch it off or change its mode in Settings > Sandbox (TUI/WebUI), `[Sandbox]` in config or `PANDO_SANDBOX=off`; inspect it with `pando sandbox status`. Windows is not enforced (Job Object containment only). See [docs/sandbox.md](docs/sandbox.md).
...
| `PANDO_SANDBOX`            | Host command sandbox override: `off`, `workspace-write`, `read-only`, `strict` (see [docs/sandbox.md](docs/sandbox.md)) |
