...
func (s *SessionRAGStore) SearchSessions(ctx context.Context, query string, limit int) ([]SessionSearchResult, error) {
	...
	vecItems, err := s.searchVector(ctx, queryEmb, subLimit)
	if err != nil {
		return nil, fmt.Errorf("sessions: vector search: %w", err)
	}

	ftsItems, err := s.searchFTS(ctx, query, subLimit)
	if err != nil {
		return nil, fmt.Errorf("sessions: fts search: %w", err)
	}

	return rrfFuse(vecItems, ftsItems, limit), nil
}
