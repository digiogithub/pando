...
func (s *KBStore) SyncDirectoryWithStats(ctx context.Context, dirPath string, deleteMissing bool) (SyncStats, error) {
	...
	for res := range results {
		...
		if _, exists := existingByPath[res.job.docPath]; !exists {
			if addErr := s.AddDocument(ctxSync, res.job.docPath, res.content, meta); addErr != nil {
				errorCount++
				if firstErr == nil {
		...
		if updateErr := s.UpdateDocument(ctxSync, res.job.docPath, res.content, meta); updateErr != nil {
			errorCount++
			if firstErr == nil {
		...
		if processed%25 == 0 {
			...
			logSyncMemStats("kb sync: progress mem", processed, jobCount)
		}
	}
