...
func (s *KBStore) SyncDirectoryWithStats(ctx context.Context, dirPath string, deleteMissing bool) (SyncStats, error) {
	...
	err = filepath.WalkDir(baseDir, func(path string, d os.DirEntry, walkErr error) error {
		...
		existingMeta, getMetaErr := s.getDocumentMetadata(ctx, docPath)
		if getMetaErr != nil {
			return fmt.Errorf("kb: check existing metadata %s: %w", docPath, getMetaErr)
		}
		if existingMeta != nil {
	...
	if deleteMissing {
		...
		for {
			docs, listErr := s.listDocumentMetadata(ctx, 200, offset)
			if listErr != nil {
