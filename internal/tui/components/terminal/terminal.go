...
func New(width, height int, shellPath string, shellArgs []string) (TerminalComponent, error) {
	...
	go func() {
		...
		for {
			...
			if n > 0 {
				logging.Debug("terminal: PTY data received", "id", id, "bytes", n)
...
func (m *terminalModel) tick() tea.Cmd {
	...
	return tea.Tick(tickInterval, func(_ time.Time) tea.Msg {
		...
		logging.Debug("terminal.tick: rendered", "id", id, "rows", len(rows), "first_row_len", len(firstRow), "first_row", firstRow)
