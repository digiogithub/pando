...
- A2. **✅ DONE 2026-06-23** (`pando/changes/delegation_a2_external_reattach.md`). Re-attach EXTERNAL delegations on parent restart (recover-completed-results): peer retains a bounded TTL result cache + a read-only `delegation.status` RPC (`DelegationProtocolVersion` 1→2); `Manager.RecoverExternalDelegation`; orchestrator persists a running breadcrumb in `tryStartWarm` (reverted on cold fallback) and `recoverStaleTasks` recovers interrupted external delegations via the injected `ExternalDelegationRecoverer` (completed→onTaskComplete pipeline, running→bounded poll, unknown→mark failed); metrics `external_reattach_recovered/_failed`; app adapter gated on `AllowExternalWarmTargets` (no new config). Default-OFF byte-identical. Manager-spawned warm children remain non-re-attachable (die with the parent). Original investigation note (now resolved for the external case) follows: Re-attach warm delegated sessions on reconnect (parent restart): on reconnect LoadSession/ResumeSession by stored ACPSessionID, re-subscribe capturing client, synthesize-fail only if resume fails; idempotent via CorrelationID. EFFORT M-L, RISK med. **DEFERRED (2026-06-22) — blocked on B2/B3.** Investigation finding: warm instances are manager-spawned stdio subprocesses (`Manager.spawnChild`, `exec.CommandContext(procCtx<-m.ctx, pandoBin, "acp")`); a parent restart closes the stdio pipes (child EOF) and cancels procCtx (SIGKILL), so a warm child CANNOT survive the parent — there is nothing to re-attach to. `EnsureInstance` also refuses external/editor instances (`ErrExternalInstance`). The mechanism A2 needs already exists (`PandoACPAgent.LoadSession` replays history, `AgentCapabilities.LoadSession=true`, SDK `conn.LoadSession`), and the warm path is fully synchronous with EPHEMERAL sessions closed immediately (`closeDelegatedSession`), persisting `ACPSessionID` only at completion — so today a mid-flight crash leaves a `running` task with no session breadcrumb and `recoverStaleTasks` just marks it failed. Net: literal cross-restart re-attach is INERT until warm targets can be surviving peers (external/IPC/server-mode), i.e. B2 (external-as-warm) and/or B3 (hot-peer IPC delegation). Re-attach should be implemented AS PART OF whichever of B2/B3 makes peers survivable, where it can actually be exercised end-to-end. Decision: defer (per user, 2026-06-22).
- A3. Queue over the per-instance cap — ✅ DONE.

## B. Routing / targeting
- B1. Target a registered project by id/name/path from spawn tool — ✅ DONE.
- B2 + B3 — ✅ DONE 2026-06-23 (merged into ONE epic per user decision: B2's "external-as-warm" cannot delegate without B3's synchronous IPC transport, so they were built together). New synchronous `delegation.run`/`delegation.cancel` IPC JSON-RPC over the existing ZeroMQ ROUTER/DEALER bus, served by a consent-gated bridge handler backed by the target's local agent (fresh ephemeral session, captures assistant text, returns it synchronously). Caller side `Manager.DelegateExternal` resolves the peer's RPC endpoint from `.pando/ipc.lock`, capability-checks via `instance.ping` (now advertises `AcceptsDelegations`+`DelegationProtocol`), runs, maps to `DelegateResult`; ctx-cancel → best-effort `delegation.cancel` by CorrelationID. Two-sided opt-in: caller `AllowExternalWarmTargets` + target `AcceptDelegations`, BOTH default OFF (behaviour byte-identical to before when off — external still `ErrExternalInstance`→cold). `WarmDelegate` gained an `allowExternal` param; `EnsureInstance`'s `ErrExternalInstance` now routes to `DelegateExternal` when opted in; new sentinels `ErrExternalDelegationRefused`/`ErrExternalUnreachable` fold into cold-fallback. Metric `external_hits` added to `DelegationMetrics`. Full TUI/WebUI/API + 7-locale i18n. Safety: never SIGTERM an editor's instance; ephemeral session isolates from the user's active session; localhost+token IPC; capability negotiation fails closed. NOTE: A2 cross-restart re-attach is now feasible for the external case (peers survive) but NOT yet implemented — left as a future follow-up under this transport. Plan `pando/plans/delegation_b3_hot_peer_ipc_epic_plan.md`; detail `pando/changes/delegation_b3_hot_peer_ipc.md`.
...
1) A1 → 2) C1+C2 → 3) B1 → 4) A3 → 5) E1 → ~~6) A2 (reconnect)~~ DEFERRED into B2/B3 → 6) ~~D1/D2~~ DONE → 7) ~~B2 + B3~~ DONE 2026-06-23 (merged epic; IPC delegation transport, external-as-warm) → 8) E2 (opportunistic) + A2 re-attach follow-up (now feasible for external peers atop the B3 transport). All knobs default-off-consistent. (A1, C1, C2, B1, A3, E1, D1, D2, B2, B3 done; A2 partially unblocked but not yet implemented. Remaining: E2; A2-external-reattach follow-up.)