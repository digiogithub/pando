...
## Hot-peer IPC delegation / external-as-warm (B2+B3, merged epic)
The warm path only reaches MANAGER-SPAWNED stdio children. An EXTERNAL (editor-launched) instance is reachable only over the IPC ZeroMQ bus, so a new synchronous `delegation.run` (+ `delegation.cancel`) JSON-RPC was added over the existing ROUTER/DEALER layer. Target side: a consent-gated bridge handler (`RegisterHandlersWithDelegation`) backed by `NewAgentDelegationRunner` (creates a fresh ephemeral session, runs the local agent turn to completion, returns the captured assistant text); `instance.ping` now advertises `AcceptsDelegations`+`DelegationProtocol` (capability negotiation, fails closed). Caller side: `Manager.DelegateExternal` resolves the peer's RPC endpoint from `.pando/ipc.lock`, capability-checks, runs `delegation.run`, maps to `DelegateResult{External:true}`; a ctx-cancel watcher fires `delegation.cancel` (by caller-owned CorrelationID) over a separate ephemeral client. `WarmDelegate` gained `allowExternal`; `EnsureInstance`'s `ErrExternalInstance` routes to `DelegateExternal` when opted in; new sentinels `ErrExternalDelegationRefused`/`ErrExternalUnreachable` fold into cold-fallback (isWarmColdFallback). Metric: `external_hits` (subset of warm_hits). Two-sided opt-in, BOTH default OFF → byte-identical legacy behaviour: caller `AllowExternalWarmTargets`, target `AcceptDelegations` (env PANDO_DELEGATION_ALLOW_EXTERNAL_WARM_TARGETS / PANDO_DELEGATION_ACCEPT_DELEGATIONS). Safety: never SIGTERM an editor instance; ephemeral session isolates from the user's active one; localhost+token IPC. delegation_b3_hot_peer_ipc.md.

## External delegation re-attach on restart (A2)
When the parent restarts mid-delegation, an external peer's delegation has usually already finished. The peer now retains recent completed results in a bounded TTL cache and answers a new read-only `delegation.status` RPC (correlation-id keyed; `DelegationProtocolVersion` bumped 1→2). On restart `recoverStaleTasks` recovers interrupted external delegations (engine=warm-acp + CorrelationID + project ref) in the background via the injected `ExternalDelegationRecoverer` (`Manager.RecoverExternalDelegation`) — completed → normal `onTaskComplete` conclusion/supervisor pipeline; still-running → bounded poll; unknown/unreachable → mark failed (pre-A2 behaviour). `tryStartWarm` persists a running breadcrumb before the blocking warm run (reverted on cold fallback) so the interrupted task is discoverable. Gated on `AllowExternalWarmTargets` (no new config); default-OFF is byte-identical. Metrics: `external_reattach_recovered`/`external_reattach_failed`. Safety: read-only (never re-runs), CorrelationID-keyed, fails closed at protocol<2, no false recovery. delegation_a2_external_reattach.md.
