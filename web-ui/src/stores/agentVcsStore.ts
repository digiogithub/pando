...
export const useAgentVcsStore = create<AgentVcsStore>((set, get) => ({
  ...
  fetchSessions: async () => {
    set({ loading: true })
    try {
      const data = await api.get<{ sessions: SessionInfo[] }>('/api/v1/agentvcs/sessions')
  ...
  fetchCommitDetail: async (commitId: string) => {
    ...
    try {
      const data = await api.get<CommitDetail>(`/api/v1/agentvcs/commits/${commitId}`)
      set({
        selectedCommit: data,
      ...
      set({ selectedCommit: null, selectedCommitDiff: [], loadingCommit: false })
    }
  },
  ...
  revertToCommit: async (commitId: string) => {
    try {
      ...
      useToastStore.getState().addToast('Reverted to commit ' + commitId.slice(0, 12), 'success')
      // Refresh commits
  ...
  revertFiles: async (commitId: string, files: string[]) => {
    try {
      ...
      useToastStore
        .getState()
        .addToast(e instanceof Error ? e.message : 'Revert files failed', 'error')
      return []
    }
  },

  selectSession: (sessionId: string) => {
    set({ selectedSessionId: sessionId, selectedCommit: null, selectedCommitDiff: [] })
    get().fetchCommits(sessionId)
  },

  clearSelection: () => {
    set({ selectedCommit: null, selectedCommitDiff: [], selectedSessionId: null, commits: [] })
  },
}))
